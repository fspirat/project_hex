...
public class NamePresetScreen extends Screen {
    ...
    public void extractRenderState(GuiGraphicsExtractor g, int mouseX, int mouseY, float delta) {
        ...
        g.text(this.font, Component.literal(HexUi.tr("name.label")), left, top + 19, 0xFFA0A0A0, false);
        HexUi.drawGradient(g, left, top + 54, W, 6, List.of(colors));
        if (renaming != null) {
            if (taken(name.strip())) {
                g.text(this.font, Component.literal(HexUi.tr("name.taken")), left, top + 64, 0xFFFF5555, false);
            }
        } else if (HexConfig.hasPreset(name.strip())) {
            g.text(this.font, Component.literal(HexUi.tr("name.will_replace")), left, top + 64, 0xFFFFD24D, false);
        } else if (HexConfig.USER_PRESETS.size() >= HexConfig.MAX_PRESETS) {
            g.text(this.font, Component.literal(HexUi.tr("name.limit", HexConfig.MAX_PRESETS)),
